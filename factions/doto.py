...
class Bloodrotter(General):
    ...
    def register_kill(self, killed: 'Entity') -> None:
        ...
        killed_max_hp = killed.max_hp if killed.max_hp is not None else 0
        self.get_healed(int(killed_max_hp * 0.25))

    def thirst(self, enemy: 'Entity') -> None:
        enemy_max_hp = enemy.max_hp if enemy.max_hp is not None else 0
        enemy_hp = enemy.hp if enemy.hp is not None else 0
        if enemy_max_hp > 0:
            self.thirst_charges = (enemy_max_hp - enemy_hp) / int(enemy_max_hp * 0.33)
        else:
            self.thirst_charges = 0
        diff = self.thirst_charges - self.prev_thirst_charges
        if diff:
            self.power = (self.power if self.power is not None else 0) + int(3 * diff)
...
class Ox(General):
    ...
    def get_attacked(self, enemy: 'Entity', power: Optional[int] = None, attack_effect: Optional[Any] = None, attack_type: Optional[str] = None) -> None:
        if not attack_type:
