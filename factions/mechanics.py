...
class Flappy(General):
    ...
    def initialize_skills(self):
        ...
        self.skills.append(Skill(self, add_path, 5, [placeholder_gobmerang], "Fire the Gobmerang!", "Launches Gobmerang to fly high in the air", Arc(self.bg, origin=(placeholder_gobmerang.x if placeholder_gobmerang != self else self.x, self.y), ratio_y=1)))
        self.skills.append(Skill(self, place_entity, 5, [Boulder(self.bg, delay=5)], "Drop the boulder!", "Tells Gobmerang to drop a boulder", SingleTarget(self.bg)))
        self.skills.append(Skill(self, place_entity, 5, [Lava(self.bg)], "Burn them from above!", "Tells Gobmerang to drop a cauldron of oil", SingleTarget(self.bg)))
        self.skills.append(Skill(self, add_path, 5, [], "Fire the Gobmerang!", "Launches Gobmerang to fly high in the air", Arc(self.bg, origin=(self.x, self.y+1), ratio_y=1, steps=120)))
        ...
        self.skills.append(Skill(self, place_entity, 5, [Explosion(self.bg, power=20)], "Last chance, boom the machine!", "Explodes the slingshot", explosion_area))
    ...
    def update(self):
        ...
        elif hasattr(self, 'slingshot') and self.slingshot and len(self.skills) > 0 and self.skills[0].cd == self.skills[0].max_cd-1 and not self.slingshot_drawn:
            self.draw_slingshot()
    ...
    def use_skill(self, i, x, y):
        ...
        if i == 0:
            if self.gobmerang.path:
                return False
            if self.slingshot.alive and i < len(self.skills) and super(Flappy, self).use_skill(i, x, y):
            ...
            if not self.gobmerang_shot and len(self.gobmerang.path) > 3:
                ...
                if i < len(self.skills) and super(Flappy, self).use_skill(i, self.gobmerang.path[index].x, self.gobmerang.path[index].y):
            ...
            if self.slingshot.alive and i < len(self.skills):
