...
class Window(object):
    def __init__(self, battleground: Battleground, side: int, host: Optional[str] = None, port: Optional[int] = None, window_id: int = 0) -> None:
        if DEBUG:
        ...
        if host is not None:
            ...
            self.network: Optional[Network] = Network(host, port)
        else:
            if DEBUG:
                sys.stdout.write("DEBUG: No network connection\n")
            self.network = None
        self.bg: Battleground = battleground
        self.side: int = side
        ...
        self.game_msgs: List[Tuple[str, Tuple[int, int, int]]] = []
        self.game_over: bool = False
        self.area_hover_color: Tuple[int, int, int] = UI_HOVER_VALID
        self.area_hover_color_invalid: Tuple[int, int, int] = (255, 0, 0) # Red for invalid
        self.default_hover_color: Tuple[int, int, int] = (200, 200, 200) # Light grey for default
        self.default_hover_function: Callable[[int, int], List[Tile]] = SingleTarget(self.bg).get_all_tiles
        self.hover_function: Optional[Callable[[int, int], Optional[List[Tile]]]] = None
    ...
    def render_all(self, grid_x, grid_y):
            ...
            for tile in self.bg.tiles.values():
                    ...
                    if drawable:
                            ...
                            if drawable.animation is not None:
